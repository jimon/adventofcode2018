...
class Map:
	...
	def __init__(self, filename, attack_elf):
		m = [x.strip() for x in open(filename).readlines()]
		self.w = len(m[0])
		self.h = len(m)
		self.attack_elf = attack_elf
	...
	def attack(self, x, y):
		dmg = self.attack_elf if self.get(x, y).t == 'G' else 3
		self.get(x, y).h -= dmg
		if self.get(x, y).h <= 0:
			self.get(x, y).t = '.'
	...
	def elfcount(self):
		return len([1 for y in range(0, self.h) for x in range(0, self.w) if self.get(x, y).t == 'E'])
...
for hitpoints in range(4, 200):
	m = Map('input1.txt', hitpoints)
	elfs = m.elfcount()
	rounds = 0
	while sim(m):
		rounds += 1
		print('------ round', rounds)
	elfs_at_end = m.elfcount()
	if elfs != elfs_at_end:
		continue
	print(hitpoints, rounds * m.allhp())
	break
